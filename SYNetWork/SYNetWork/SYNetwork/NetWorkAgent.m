...
//                SYLog(@"Error, class of key/value in headerFieldValueDictionary should be NSString.");
            }
        }
    }
...
//            SYLog(@"Error, unsupport method type");
            return;
        }
    }
...
//    SYLog(@"Finished Request: %@", NSStringFromClass([request class]));
    if (request) {
...
//            SYLog(@"Request %@ failed, status code = %ld",
...
//    SYLog(@"Request queue size = %lu", (unsigned long)[_requestsRecord count]);
}
@end
